...
- Jump 内核：⇧⌘G → PathTextField → directed click → Return；不代 Open/Save
 - 侧栏列表：**Rec | Fav | Find | Zox** segment；单行 `名字 + path`；截断显示 `…`
 - Recents/Finder/Zox 行：★ 收藏、⎘ 复制全路径；Favorites：↑↓✕
- **Finder tab**：点 ↻ 拉打开的 Finder 窗路径（AppleScript + Automation）；`capacity = 50`
- **Zoxide tab**：点 ↻ 跑 `zoxide query -l`（Homebrew 等常见 path）；`capacity = 50`；行同 Recents
